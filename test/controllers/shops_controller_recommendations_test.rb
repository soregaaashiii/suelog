...
class ShopsControllerRecommendationsTest < ActiveSupport::TestCase
  ...
  test "shared hydration loads unique shops once and restores every group order" do
    origin = insert_shop(latitude: 34.6937, longitude: 135.5023, genre: "バー")
    shops = 18.times.map do |index|
      insert_shop(
    ...
    insert_clicks(shops.fetch(12), 7)

    groups = {
      nearby: recommendation_selection(shops.values_at(0, 1, 2, 3, 4, 5), radius_km: 3.0),
      same_genre: recommendation_selection(shops.values_at(6, 7, 8, 9, 10, 11), radius_km: 5.0),
      popular: recommendation_selection(
        shops.values_at(12, 13, 14, 15, 16, 17),
        radius_km: 5.0,
        clicks_count_by_id: { shops.fetch(12).id => 7 }
      )
    }

    sql = capture_sql do
      @hydrated_groups = @controller.send(:load_recommendation_shop_groups, groups, origin_shop: origin)
    end
    ...
    assert_equal 4, recommendation_attachment_queries(sql).size
    assert_operator recommendation_blob_queries(sql).size, :<=, 4
    assert @hydrated_groups.values.flatten.all? { |shop| recommendation_images_loaded?(shop) }
  end

  test "shared hydration reuses one shop object across two and three groups" do
    origin = insert_shop(latitude: nil, longitude: nil, genre: "バー")
    shared_by_two = insert_shop(latitude: nil, longitude: nil, genre: "バー")
    shared_by_three = insert_shop(latitude: nil, longitude: nil, genre: "バー")
    nearby_only = insert_shop(latitude: nil, longitude: nil, genre: "バー")
    same_only = insert_shop(latitude: nil, longitude: nil, genre: "バー")
    popular_only = insert_shop(latitude: nil, longitude: nil, genre: "バー")

    groups = {
      nearby: recommendation_selection([ shared_by_two, nearby_only, shared_by_three ]),
      same_genre: recommendation_selection([ shared_by_three, same_only, shared_by_two ]),
      popular: recommendation_selection(
        [ popular_only, shared_by_three ],
        clicks_count_by_id: { popular_only.id => 9, shared_by_three.id => 4 }
      )
    }

    hydrated = @controller.send(:load_recommendation_shop_groups, groups, origin_shop: origin)
  ...
  test "shared hydration preserves attached image selection and rendered html" do
    origin = insert_shop(latitude: nil, longitude: nil, genre: "バー")
    with_image = insert_shop(latitude: nil, longitude: nil, genre: "バー")
    without_image = insert_shop(latitude: nil, longitude: nil, genre: "バー")
    with_image.food_photos.attach(
      io: File.open(Rails.root.join("app/assets/images/fallbacks/bar.jpg")),
      filename: "bar.jpg",
      content_type: "image/jpeg"
    )
    groups = {
      nearby: recommendation_selection([ with_image, without_image ]),
      same_genre: recommendation_selection([ without_image, with_image ]),
      popular: recommendation_selection(
        [ with_image ],
        clicks_count_by_id: { with_image.id => 3 }
      )
    }

    expected = hydrate_groups_individually(groups, origin)
    actual = @controller.send(:load_recommendation_shop_groups, groups, origin_shop: origin)

    groups.each_key do |group_name|
      assert_shop_results_equal(
        expected.fetch(group_name),
        actual.fetch(group_name),
        compare_clicks: group_name == :popular
      )
      expected_html = render_cards(expected.fetch(group_name))
      actual_html = render_cards(actual.fetch(group_name))
      assert_equal expected_html, actual_html
      assert_equal Digest::SHA256.hexdigest(expected_html), Digest::SHA256.hexdigest(actual_html)
    end
    assert actual.fetch(:nearby).first.food_photos_attachments.loaded?
    assert actual.fetch(:nearby).first.food_photos.first.blob.present?
    assert actual.fetch(:nearby).first.food_photos.first.variable?
    assert_empty actual.fetch(:nearby).last.food_photos
  end
  ...
  def recommendation_selection(shops, radius_km: nil, clicks_count_by_id: nil)
    ShopsController::RecommendationSelection.new(
      ranked_shops: shops.map { |shop| ShopsController::RecommendationRankedShop.new(id: shop.id) },
      radius_km: radius_km,
      clicks_count_by_id: clicks_count_by_id
    )
  end

  def hydrate_groups_individually(groups, origin)
    groups.transform_values do |selection|
      @controller.send(
        :load_recommendation_shop_groups,
        { result: selection },
        origin_shop: origin
  ...
  def recommendation_display_queries(sql)
    sql.select do |statement|
      statement.match?(/FROM ["`]?shops["`]?/i) && statement.match?(/WHERE .*["`]?id["`]? IN \(/i)
    end
  end

  def recommendation_attachment_queries(sql)
    sql.select { |statement| statement.include?("active_storage_attachments") }
  end

  def recommendation_blob_queries(sql)
    sql.select { |statement| statement.include?("active_storage_blobs") }
  end

  def recommendation_images_loaded?(shop)
    %i[
      food_photos_attachments
      interior_photos_attachments
      exterior_photos_attachments
      menu_photos_attachments
    ].all? { |association| shop.association(association).loaded? }
  end
