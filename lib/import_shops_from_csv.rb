# /Users/kawamuratakuya/Desktop/吸えログデータ/dev/suelog/lib/import_shops_from_csv.rb
require "csv"

CSV_PATH = File.expand_path("../tmp/shops_google.csv", __dir__)
...
CSV.foreach(CSV_PATH, headers: true).with_index(2) do |row, lineno|
  begin
    ...
    # ✅ 追加したい項目（CSVにあれば採用。無ければ不明扱い）
    opening_hours   = safe_strip(row["opening_hours"])      # 無いなら nil
    nearest_station = safe_strip(row["nearest_station"])    # 無いなら nil

    # 喫煙（CSVの smoking_type / smoking_area どっちでも拾えるようにする）
