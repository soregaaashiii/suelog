...
return true if scope.where(place_id: place_id_value).exists?
end

if normalized_phone.present?
return true if scope.where(normalized_phone: normalized_phone).exists?
end
...
scope.order(created_at: :desc).limit(5000).pluck(:id, :name, :address).any? do |_candidate_id, candidate_name, candidate_address|
...
Rails.logger.warn("[duplicate_flag_present?] #{e.class}: #{e.message}")
false
end
